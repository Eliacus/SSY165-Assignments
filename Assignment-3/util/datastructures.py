...
class PetriNet(object):
    ...
    def make_reachability_graph(self):
        ...
        nodes = {str(self.init_marking.flatten())}
        edges = set()
        ...
        while new_markings:
            ...
            for i, t in enumerate(self.T):
                ...
                if np.all(m >= np.matmul(self.A_minus, s)):
                    ...
                    m_plus_str = str(m_plus.flatten())
                    edges.add(Edge(str(m.flatten()), m_plus_str, t))
                    if m_plus_str not in nodes:
                        nodes.add(m_plus_str)
                        new_markings.append(m_plus)
        return DiGraph(nodes, str(self.init_marking), edges)